...
DATASET_COLUMNS = [
    ...
    # Graph centrality metrics (novel contribution)
    "degree_centrality",
...
def build_dataset_row(
    ...
    """
    Build the final dataset row in the correct schema order.

    Inputs:
    - cluster_id, namespace, service_name → identification info
    - window_size_seconds → scraping time range
    - merged_metrics → node/pod/service/mesh metrics
    - centrality_for_service → graph centrality metric values
    - scaling_decision → current and recommended replicas + action
    - timestamp → optional override (default = current UTC time)
    """

    ts = timestamp or current_utc_iso()

    # Calculate stress index based on merged metrics
    stress = compute_stress_index(merged_metrics)

    # Graph centrality values for this service
    degree = centrality_for_service.get("degree_centrality", 0.0)
    between = centrality_for_service.get("betweenness_centrality", 0.0)
    close = centrality_for_service.get("closeness_centrality", 0.0)
    eigen = centrality_for_service.get("eigenvector_centrality", 0.0)
    ...
    # Ensure missing fields do not cause data corruption
